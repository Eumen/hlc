...
<div class="accounttitle"><h4>Bonus Detail  [<a href='javascript:history.back()'>Index</a>] </h4></div>
<table width="100%" class="tab3" border="0" cellpadding="3" cellspacing="1" id="tb1" bgcolor="#b9c8d0">
	<thead>
		<tr>
			<th><span>Date</span></th>
			<th <?php echo ($c_b[1]); ?>><span>Bonus</span></th>
			<th <?php echo ($c_b[12]); ?>><span>Detail</span></th>
		</tr>
	</thead>
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center">
		...
		<td <?php echo ($c_b[12]); ?>><a href="__URL__/financeShow/RDT/<?php echo ($vo['s_date']); ?>/PDT/<?php echo ($vo['e_date']); ?>/UserID/<?php echo ($user_id); ?>" title="这一期明细">Detail</a></td>
...
<table width="100%" class="tab3_bottom" border="0" cellpadding="0" cellspacing="1">
    <tr>
    	<td width="50%"><form id="form1" name="form1" method="post" action="__URL__/financeTable">Date：<input name="FanNowDate" type="text" id="FanNowDate" onFocus="showCalendar(this)" readonly /> <input type="submit" name="Submit" value="Query" class="button_text" />
          <input name="UserID" type="hidden" value="<?php echo ($user_id); ?>" /></form></td>
