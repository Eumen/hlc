...
class PublicAction extends CommonAction
{
    ...
    public function checkLogin()
    {
        if (empty($_POST['account'])) {
            $this->error('please input your account！');
        } elseif (empty($_POST['password'])) {
            $this->error('please input your password！');
        } elseif (empty($_POST['verify'])) {
            $this->error('please input the captcha！');
        }
        $fee = M('fee');
        // $sel = (int) $_POST['radio'];
        // if($sel <=0 or $sel >=3){
        ...
        // 使用用户名、密码和状态的方式进行认证
        if (false == $authInfo) {
            $this->error('the account is not exist or locked！');
        } else {
            ...
            if ($authInfo['is_pay'] < 1) {
                $this->error('The user has not been opened and cannot log on to the system！');
                exit();
            }
            if ($authInfo['is_lock'] != 0) {
                $this->error('The user is locked, please contact the administrator！');
                exit();
            }
